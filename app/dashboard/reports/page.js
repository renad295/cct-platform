...
const usersData = await res.json()
  ...
  const existing = worked.filter(c => c.status === "Existing Client").length
  const confirmed = assigned.filter(c => c.is_confirmed).length
  const meetingRate = total > 0 ? ((meetings / total) * 100).toFixed(1) : 0
  const opportunityRate = total > 0 ? ((opportunities / total) * 100).toFixed(1) : 0
  const successRate = total > 0 ? (((meetings + opportunities) / total) * 100).toFixed(1) : 0
  return { total, newClients, meetings, opportunities, notInterested, noAnswer, existing, confirmed, meetingRate, opportunityRate, successRate, totalAssigned: assigned.length }
}

  const getRating = (stats) => {
    const rate = parseFloat(stats.meetingRate)
    if (rate >= 15) return { label: "Excellent", color: "bg-green-50 text-green-700 border-green-200", dot: "bg-green-500" }
    if (rate >= 8) return { label: "Good", color: "bg-blue-50 text-blue-700 border-blue-200", dot: "bg-blue-500" }
    return { label: "Needs Improvement", color: "bg-red-50 text-red-700 border-red-200", dot: "bg-red-500" }
  }
...
Write a performance summary covering: current performance level, key strengths, main challenge, and one specific actionable recommendation to improve conversion rates.`
  ...
  const totalMeetings = clients.filter(c => c.status === "Meeting Arranged").length
  const totalOpportunities = clients.filter(c => c.status === "Opportunity").length
  const totalNotInterested = clients.filter(c => c.status === "Not Interested").length
  ...
  // Industry breakdown
  const industryData = clients.reduce((acc, c) => {
  ...
  const leaderboard = activeUsers.map(u => {
    const stats = getEmployeeStats(u.email)
    return { user: u, stats, name: u.user_metadata?.full_name || u.email?.split("@")[0] }
  }).sort((a, b) => parseFloat(b.stats.meetingRate) - parseFloat(a.stats.meetingRate))
...
{ label: "Existing Clients", value: totalExisting, color: "#7c3aed", sub: "already a client" },          ].map((s, i) => (
  ...
  <PieChart>
    <Pie data={pieData} cx="35%" cy="50%" innerRadius={60} outerRadius={100}
    ...
    />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={monthlyData}>
                ...
                <Line type="monotone" dataKey="meetings" stroke="#0C447C" strokeWidth={2} dot={false} name="Meetings" />
                <Line type="monotone" dataKey="opportunities" stroke="#27ae60" strokeWidth={2} dot={false} name="Opportunities" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          {/* Employee Bar */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">Employee Comparison</h2>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={barData} barSize={10}>
                ...
                <Bar dataKey="No Answer" fill="#f59e0b" radius={4} />
                <Bar dataKey="Not Interested" fill="#ef4444" radius={4} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          {/* Industry Breakdown */}
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">Industry Breakdown</h2>
            {industryData.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-10">No industry data yet</p>
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={industryData} barSize={10} layout="vertical">
                  ...
                  <Bar dataKey="meetings" fill="#0C447C" radius={4} name="Meetings" />
                  <Bar dataKey="opportunities" fill="#27ae60" radius={4} name="Opportunities" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
            ...
            {leaderboard.map((item, index) => {
              ...
              return (
                <div key={item.user.id} className="flex items-center gap-4">
                  <span className={`w-6 text-sm font-bold ${index === 0 ? "text-amber-500" : index === 1 ? "text-gray-400" : index === 2 ? "text-amber-700" : "text-gray-300"}`}>
                    #{index + 1}
                  </span>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-400">{item.stats.total} clients</span>
                        <span className="text-xs font-semibold text-blue-700">{item.stats.meetingRate}% rate</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full border ${rating.color}`}>{rating.label}</span>
                      </div>
                    </div>
                    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full transition-all"
                        style={{ width: `${Math.min(parseFloat(item.stats.meetingRate) * 4, 100)}%` }}></div>
                    </div>
                  </div>
                </div>
              )
        ...
        {/* Employee Cards */}
        <h2 className="text-sm font-semibold text-gray-900 mb-4">Team Members — Click for AI Analysis</h2>
...
<p className="text-xs text-gray-400">{stats.totalAssigned} clients</p>                    </div>
                  </div>
                  <span className={`text-xs font-medium px-2 py-1 rounded-full border ${rating.color}`}>
                    {rating.label}
                  </span>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center mb-3">
                  <div className="bg-blue-50 rounded-xl py-2">
                    <p className="text-sm font-bold text-blue-700">{stats.meetings}</p>
                    <p className="text-xs text-gray-400">Meetings</p>
                  </div>
                  <div className="bg-green-50 rounded-xl py-2">
                    <p className="text-sm font-bold text-green-700">{stats.opportunities}</p>
                    <p className="text-xs text-gray-400">Opport.</p>
                  </div>
                  <div className="bg-gray-50 rounded-xl py-2">
                    <p className="text-sm font-bold text-gray-700">{stats.meetingRate}%</p>
                    <p className="text-xs text-gray-400">Rate</p>
                  </div>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-blue-500 to-green-500 rounded-full"
                    style={{ width: `${Math.min(parseFloat(stats.successRate) * 2, 100)}%` }}></div>
                </div>
              </button>
            )
          })}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-red-50 text-red-700 text-sm font-bold flex items-center justify-center">
                  {(selectedUser.user.user_metadata?.full_name || selectedUser.user.email?.split("@")[0])?.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h2 className="text-base font-semibold text-gray-900">
                    {selectedUser.user.user_metadata?.full_name || selectedUser.user.email?.split("@")[0]}
                  </h2>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: "Total", value: selectedUser.stats.total, color: "text-gray-900", bg: "bg-gray-50" },
                  { label: "Meetings", value: selectedUser.stats.meetings, color: "text-blue-700", bg: "bg-blue-50" },
                  { label: "Opportunities", value: selectedUser.stats.opportunities, color: "text-green-700", bg: "bg-green-50" },
                  { label: "No Answer", value: selectedUser.stats.noAnswer, color: "text-amber-600", bg: "bg-amber-50" },
                  { label: "Not Interested", value: selectedUser.stats.notInterested, color: "text-red-600", bg: "bg-red-50" },
                  { label: "Success Rate", value: `${selectedUser.stats.successRate}%`, color: "text-purple-700", bg: "bg-purple-50" },
                ].map((s, i) => (
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                {loadingAnalysis ? (
                  <div className="flex items-center gap-1.5">
                    ...
                    <div className="w-2 h-2 rounded-full bg-red-700 animate-bounce" style={{ animationDelay: "0.1s" }}></div>
                    <div className="w-2 h-2 rounded-full bg-red-700 animate-bounce" style={{ animationDelay: "0.2s" }}></div>
                  </div>
                ) : (
